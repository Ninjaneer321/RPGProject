...
public class WowCameraScript : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (!playerManager.InteractWithUI())
        {
            ...
            if (GUIUtility.hotControl == 0)
            {
                ...
                {
                    playerManager.GetComponent<PlayerLocomotion>().cameraObject = Camera.main.transform;
                }
                ...
                {
                    //playerManager.GetComponent<PlayerLocomotion>().cameraObject = newTarget;
                    xDeg += Input.GetAxis("Mouse X") * xSpeed * 0.04f;
                    yDeg -= Input.GetAxis("Mouse Y") * ySpeed * 0.04f;
                }
        ...
        Vector3 trueTargetPosition = new Vector3(target.position.x, target.position.y, target.position.z) - vTargetOffset;
            ...
            //if (!playerManager.GetComponent<PlayerLocomotion>().isGrounded)
            //{
            //    position.y = Mathf.Clamp(position.y, position.y, transform.position.y);
            //    Debug.Log("Clamp Y Here Somehow!");
            //}
