...
namespace GameDevTV.Utils.Editor
{
    ...
    public class PredicatePropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (selectedPredicate == EPredicate.Select) return;
            ...
            if (selectedPredicate == EPredicate.HasQuest || selectedPredicate == EPredicate.CompletedQuest || selectedPredicate == EPredicate.CompletedObjective 
                || selectedPredicate == EPredicate.HasNotCompletedQuest)
            {
            ...
            if (selectedPredicate == EPredicate.MinimumLevel)
            {
                position.y += propHeight;
                DrawIntSlider(position, "Minimum", parameterZero, 1, 100);
            }
            if (selectedPredicate == EPredicate.HasKilled)
            {
                position.y += propHeight;
                DrawStringField(position, "Enemy Identifier", parameterZero);
                position.y += propHeight;
                DrawIntSlider(position, "Minimum", parameterOne, 1, 100);
            }
            if (selectedPredicate == EPredicate.MinimumHealthPercentageToUseAbility)
            {
                position.y += propHeight;
                DrawIntSlider(position, "Minimum", parameterOne, 1, 100);
            }
            ...
            {
                position.y += propHeight;
                DrawStat(position, parameterZero);
                position.y += propHeight;
                DrawIntSlider(position, "Minimum", parameterOne, 1, 100);
            }
            position.y += propHeight;
        ...
        void DrawStat(Rect position, SerializedProperty element)
        {
            if (!Enum.TryParse(element.stringValue, out Stat stat))
            {
                stat = Stat.Strength;
            }

            EditorGUI.BeginProperty(position, new GUIContent("Stat"), element);
            Stat newStat = (Stat)EditorGUI.EnumPopup(position, "Stat:", stat);
            if (newStat != stat)
            {
                element.stringValue = $"{newStat}";
            }
            EditorGUI.EndProperty();
        }
        ...
        void DrawInventoryItemList(Rect position, SerializedProperty element, bool stackable = false, bool equipment = false)
        {
            ...
            EditorGUI.BeginProperty(position, new GUIContent("itens"), element);
        ...
        private void DrawStringField(Rect position, string caption, SerializedProperty element)
        {
            EditorGUI.BeginProperty(position, new GUIContent(caption), element);
            EditorGUI.BeginChangeCheck();
            string result = EditorGUI.TextField(position, caption, element.stringValue);
            if (EditorGUI.EndChangeCheck())
            {
                element.stringValue = $"{result}";
            }
            EditorGUI.EndProperty();
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            {
                case EPredicate.Select:
                    return propHeight;
                //case EPredicate.HasLevel: //All of these take 1 parameter
                case EPredicate.MinimumLevel:
                case EPredicate.HasKilled:
                case EPredicate.MinimumHealthPercentageToUseAbility:
                case EPredicate.CompletedQuest:
                case EPredicate.HasQuest:
                case EPredicate.HasItem:
                case EPredicate.HasItemEquipped: //Predicate + 1 parameter + negate
